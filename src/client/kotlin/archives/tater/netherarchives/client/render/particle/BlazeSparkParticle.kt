...
class BlazeSparkParticle(level: ClientLevel, x: Double, y: Double, z: Double, scale: Float, private val spriteProvider: SpriteSet) :
    SingleQuadParticle(level, x, y, z, spriteProvider.first()) {
        init {
            this.quadSize = scale
    ...
    class Factory(private val spriteProvider: SpriteSet) : ParticleProvider<SimpleParticleType> {
        override fun createParticle(
            ...
            zAux: Double,
            random: RandomSource
        ): Particle = BlazeSparkParticle(level, x, y, z, 0.25f, spriteProvider)
    }
    ...
    class SmallFactory(private val spriteProvider: SpriteSet) : ParticleProvider<SimpleParticleType> {
        override fun createParticle(
            ...
            velocityZ: Double,
            random: RandomSource
        ): Particle = BlazeSparkParticle(level, x, y, z, 0.125f, spriteProvider)
    }
}
